...
impl Picross {
    pub fn fill_possibles(&mut self) {
        self.possible_cols = (0..self.length).map(|i| gen_picross_rows(self.height, &self.col_spec[i])).collect();
        self.possible_rows = (0..self.height).map(|i| gen_picross_rows(self.length, &self.row_spec[i])).collect();
    }
}
